...
# ├── ev_use/
# │   └── networks/*.nc
ROOT_DIR = Path("results/sensitivity_2050")
...
CONFIG_YAML = Path("config/sensitivity_parameters/config.yaml")
...
def main():
    ...
    with pd.ExcelWriter(OUTPUT_EXCEL, engine="openpyxl") as writer:
        wide.to_excel(writer, sheet_name="levels", index=False)
        vs_base.to_excel(writer, sheet_name="vs_base", index=False)
